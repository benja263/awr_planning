...
class ContinuousCritic(BaseModel):
    """
    Critic network(s) for AWR.
    It represents the state value function .
    Compared to A2C/PPO critics, this one represents the Value Function
    and takes the continuous action as input. It is concatenated with the state
    and then fed to the network which outputs a single value: V(s).
...
class Actor(BasePolicy):
    ...
    def __init__(
        ...
        latent_dim_pi = net_arch[-1] if len(net_arch) > 0 else features_dim
...
class AWRPolicy(BasePolicy):
    """
    Policy class (with both actor and critic) for AWR.
    ...
    def __init__(
        ...
        if isinstance(net_arch, list):
            ...
            assert "vf" in net_arch, "Error: no key 'vf' was provided in net_arch for the critic network"
    ...
    def _build(self, lr_schedule: Schedule) -> None:
        ...
        self.actor.optimizer = self.optimizer_class(
            ...
            lr=self.actor_lr,  # type: ignore[call-arg]
        ...
        self.critic.optimizer = self.optimizer_class(
            critic_parameters,
            lr=self.critic_lr,  # type: ignore[call-arg]
            **self.optimizer_kwargs,
        )
